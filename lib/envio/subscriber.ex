defmodule Envio.Subscriber do
  ...
  defmacro __using__(opts \\ []) do
    quote bind_quoted: [opts: opts] do
      ...
      @namespace Macro.underscore(@module)
      @fq_joiner "."
      ...
      def subscribe([_ | _] = channels),
        do: GenServer.call(@module, {:subscribe, channels})
      ...
      `GenServer` might be started by invoking `#{@module}.start_link`.
      """
      @spec start_link(list()) ::
              {:ok, pid()} | :ignore | {:error, {:already_started, pid()} | term()}
      def start_link(opts \\ []),
        do: GenServer.start_link(@module, %Envio.State{options: opts}, name: @module)
      ...
      def handle_call({:subscribe, [_ | _] = channels}, _from, %Envio.State{} = state) do
        with {:ok, %Envio.State{subscriptions: subscriptions}} <- do_subscribe(channels, state) do
          {:reply, subscriptions[@module], state}
        else
      ...
      defp do_subscribe(channels, %Envio.State{} = state) do
        ...
        channels = Envio.Channels.register(@module, channels)
        {:ok, %Envio.State{state | subscriptions: %{@module => channels}}}
      end
    end
  end
end
